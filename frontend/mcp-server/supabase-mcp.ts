...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case "query_table": {
        const { table, select = "*", filter, order, limit, offset } = args as any;
      ...
      case "update_row": {
        const { table, filter, data } = args as any;
      ...
      case "delete_row": {
        const { table, filter } = args as any;
      ...
      case "list_tables": {
        ...
        return {
          content: [
            {
              type: "text",
              text: JSON.stringify(data?.map((t: any) => t.table_name) || [], null, 2),
            },
          ],
        };
      }
